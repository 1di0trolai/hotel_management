...
INSERT INTO Role (RoleTitle, RoleDesc)
SELECT 'Concierge', 'Assists guests with bookings, requests and local information'
WHERE NOT EXISTS (SELECT 1 FROM Role WHERE RoleTitle = 'Concierge');

INSERT INTO Role (RoleTitle, RoleDesc)
SELECT 'Bartender', 'Manages bar service and bar charges on guest bills'
WHERE NOT EXISTS (SELECT 1 FROM Role WHERE RoleTitle = 'Bartender');

INSERT INTO Role (RoleTitle, RoleDesc)
SELECT 'Cashier', 'Manages restaurant charges and processes guest payments'
WHERE NOT EXISTS (SELECT 1 FROM Role WHERE RoleTitle = 'Cashier');
GO
...
INSERT INTO Permission (PermissionKey, PermissionDesc)
SELECT 'manage_bar_charges', 'Add bar charges to a bill' WHERE NOT EXISTS (SELECT 1 FROM Permission WHERE PermissionKey = 'manage_bar_charges');

INSERT INTO Permission (PermissionKey, PermissionDesc)
SELECT 'manage_restaurant_charges', 'Add restaurant charges to a bill' WHERE NOT EXISTS (SELECT 1 FROM Permission WHERE PermissionKey = 'manage_restaurant_charges');
...
WHERE r.RoleTitle = 'Manager'
  AND p.PermissionKey IN (
      ...
      'view_guests', 'manage_employees', 'view_statistics'
  )
  AND NOT EXISTS (SELECT 1 FROM RolePermission rp WHERE rp.RoleID = r.RoleID AND rp.PermissionID = p.PermissionID);
